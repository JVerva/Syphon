...
public class PlayerMovement : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        //for controlling player while in the air
        if (!characterController.isGrounded)
        {
            characterController.Move(new Vector3(aerialMoveVelocity.x, 0, aerialMoveVelocity.y) * Time.deltaTime);
        }
        RotateCharacter();
    }
    ...
    private void GetVelocity()
    {
        ...
        {
            Vector2 horizontalSpeed = Vector2.Lerp(new Vector2(velocity.x, velocity.z), Vector2.zero, airResistance / new Vector2(velocity.x, velocity.z).magnitude * Time.deltaTime);
            ...
            velocity = new Vector3(horizontalSpeed.x, verticalSpeed, horizontalSpeed.y);
            return;
        }
    ...
    {
        if (moveSpeed < playerStats.runSpeed&&characterController.isGrounded)
